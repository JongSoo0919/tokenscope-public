...
function MetaRow({ fix, confidence }: { fix: Fix; confidence?: "HIGH" | "MEDIUM" | "LOW" }) {
  ...
  const target = fix.action.kind === "edit_config_md"
    ? fix.action.provider === "gemini" ? "GEMINI.md" : fix.action.provider === "codex" ? "AGENTS.md" : fix.action.provider === "cursor" ? "Cursor Rules" : fix.action.provider === "wiki" ? "Wiki RAG" : "CLAUDE.md"
    : "프로젝트 AGENTS.md";
